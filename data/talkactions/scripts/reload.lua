local reloadTypes = {
	...
	["modules"] = { targetType = 20, name = "modules"},
...
function onSay(player, words, param)
	...
	local messaType = player:getClient().os == CLIENTOS_FLASH and MESSAGE_STATUS_WARNING or MESSAGE_STATUS_CONSOLE_BLUE
 	if not reloadType then
 		player:sendTextMessage(messaType, "Reload type not found.")
 		return false
 	end
 
 	Game.reload(reloadType.targetType)
	saveServer()
 	player:sendTextMessage(messaType, string.format("Reloaded %s and saved game.", reloadType.name))
 	return true
end